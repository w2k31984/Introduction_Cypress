...
async function makeServer(_a = {}) {
    var _b;
    var { debug = false, idleTimeout = IDLE_TIMEOUT } = _a, serverConfig = __rest(_a, ["debug", "idleTimeout"]);
    const { server, port } = await (0, handler_1.makeHandler)(serverConfig);
    console.log(port); // NOTE: this is a part of the generic protocol
    (_b = process.send) === null || _b === void 0 ? void 0 : _b.call(// NOTE: this is a part of the generic protocol
    process, { name: 'port', payload: { port } }); // NOTE: this is a part of the js specific protocol
    if (!server) {
